        ...
        <div className="project-grid">
          {projects.map((project) => (
            <button className={`project-card ${project.art}`} data-motion-card key={project.number} onClick={() => { setSelectedMasonryItem(null); setActiveProject(project) }}>
              <div className="project-art" data-motion-media aria-hidden="true">{project.video ? <video className="project-image project-video" src={project.video} autoPlay loop muted playsInline preload="metadata" /> : project.image && <img className="project-image" src={project.image} alt="" loading="lazy" />}<span className="project-image-overlay" /><span className="art-label"><b>{project.number}</b><span> / {project.category}</span></span><span className="art-glow" /><span className="art-shape shape-a" /><span className="art-shape shape-b" /><span className="art-grid-lines" /></div>
